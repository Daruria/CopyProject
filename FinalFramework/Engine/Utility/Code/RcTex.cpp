...
HRESULT CRcTex::Ready_Buffer(RC_TEX_POS ePos)
{
	m_dwVtxSize = sizeof(VTXTEX);
	m_dwVtxCnt = 4;
	m_dwVtxFVF = FVF_TEX;
	...
	switch (ePos)
	{
	case Engine::CRcTex::RTS_LEFT_TOP:		vStartPoint = { 0.f,	0.f };		break;
	case Engine::CRcTex::RTS_CENTER_TOP:	vStartPoint = { -0.5f,	0.f };		break;
	case Engine::CRcTex::RTS_RIGHT_TOP:		vStartPoint = { -1.f,	0.f };		break;
	case Engine::CRcTex::RTS_LEFT_CENTER:	vStartPoint = { 0.f,	0.5f };	break;
	case Engine::CRcTex::RTS_CENTER_CENTER:	vStartPoint = { -0.5f,	0.5f };	break;
	case Engine::CRcTex::RTS_RIGHT_CENTER:	vStartPoint = { -1.f,	0.5f };	break;
	case Engine::CRcTex::RTS_LEFT_BOTTOM:	vStartPoint = { 0.f,	1.f };		break;
	case Engine::CRcTex::RTS_CENTER_BOTTOM:	vStartPoint = { -0.5f,	1.f };		break;
	case Engine::CRcTex::RTS_RIGHT_BOTTOM:	vStartPoint = { -1.f,	1.f };		break;
	}
...
void CRcTex::Set_LocalPos(const _vec3 * pLeftTop, const _vec3 * pRightTop, const _vec3 * pRightBottom, const _vec3 * pLeftBottom)
{
	VTXTEX* pVertex = nullptr;
...
void CRcTex::Set_LocalPos(RC_TEX_POS ePos)
{
	_vec2 vStartPoint;
...
void CRcTex::Set_UV(const _vec2 * pLeftTop, const _vec2 * pRightTop, const _vec2 * pRightBottom, const _vec2 * pLeftBottom)
{
	VTXTEX* pVertex = nullptr;
...
void CRcTex::Set_UV(const _vec2 * pLeftTop, const _float & fU, const _float & fV)
{
	VTXTEX* pVertex = nullptr;
...
void CRcTex::Set_UV(RC_TEX_POS ePos, const _float & fU, const _float & fV)
{
	VTXTEX* pVertex = nullptr;
	...
	case Engine::CRcTex::RTS_LEFT_CENTER:
		...
		break;
	case Engine::CRcTex::RTS_CENTER_CENTER:
		...
		break;
	case Engine::CRcTex::RTS_RIGHT_CENTER:
	...
	case Engine::CRcTex::RTS_CENTER_BOTTOM:
		...
		break;
	case Engine::CRcTex::RTS_RIGHT_BOTTOM:
...
void CRcTex::Set_UV(RC_TEX_POS ePos, const _vec2 * pUV)
{
...
CRcTex * CRcTex::Create(LPDIRECT3DDEVICE9 pGraphicDev, RC_TEX_POS ePos)
{
	CRcTex* pInst = new CRcTex(pGraphicDev);

	if (FAILED(pInst->Ready_Buffer(ePos)))
		Safe_Release(pInst);
