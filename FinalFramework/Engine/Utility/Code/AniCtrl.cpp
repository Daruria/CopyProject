...
void Engine::CAniCtrl::Set_AnimationSet(const _uint& iIndex, const _double& dInterpolationTime)
{
	if (m_iOldAniIdx == iIndex)
		return;
	
	m_dInterpolationTime = dInterpolationTime;
	...
	m_pAniCtrl->KeyTrackEnable(m_iCurrentTrack, FALSE, m_fAccTime + m_dInterpolationTime);

	// 인자값으로 들어오는 트랙에 세팅된 애니메이션 셋을 어떤 속도로 움직일 것인지 설정하는 함수(속도의 상수 값은 1)
	m_pAniCtrl->KeyTrackSpeed(m_iCurrentTrack, 1.f, m_fAccTime, m_dInterpolationTime, D3DXTRANSITION_LINEAR);

	// 인자값으로 들어오는 트랙의 가중치를 설정하는 함수
	m_pAniCtrl->KeyTrackWeight(m_iCurrentTrack, 0.1f, m_fAccTime, m_dInterpolationTime, D3DXTRANSITION_LINEAR);


	// 0번 트랙의 활성화를 지시하는 함수
	m_pAniCtrl->SetTrackEnable(m_iNewTrack, TRUE);
	m_pAniCtrl->KeyTrackSpeed(m_iNewTrack, 1.f, m_fAccTime, m_dInterpolationTime, D3DXTRANSITION_LINEAR);
	m_pAniCtrl->KeyTrackWeight(m_iNewTrack, 0.9f, m_fAccTime, m_dInterpolationTime, D3DXTRANSITION_LINEAR);
...
void CAniCtrl::Set_AnimationPosition(const _double & dPosition)
{
...
_uint CAniCtrl::Get_AnimationMax()
{
	return m_pAniCtrl->GetMaxNumAnimationSets();
}

_double CAniCtrl::Get_AnimationProgress()
{
...
_double CAniCtrl::Get_AnimationPosition()
{
	D3DXTRACK_DESC		tTrackInfo;
	ZeroMemory(&tTrackInfo, sizeof(D3DXTRACK_DESC));
