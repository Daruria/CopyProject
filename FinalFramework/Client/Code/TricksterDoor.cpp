...
CTricksterDoor::CTricksterDoor(LPDIRECT3DDEVICE9 pGraphicDev)
	:	CDynamicObject(pGraphicDev)
{
}

CTricksterDoor::~CTricksterDoor()
{
}

HRESULT CTricksterDoor::Ready_GameObject(const _ushort& wContainerIdx, const wstring& wstrBufferTag, const wstring& wstrName, vector<_ulong>* pNaviIndices)
{
	...
	m_eCurState = CLOSED;
	for (auto& dwIndex : *pNaviIndices)
	{
		m_pNaviCom->Change_CellOption(dwIndex, Engine::CCell::NO_MOVE);
	}

	return S_OK;
}

HRESULT CTricksterDoor::Add_Component(const _ushort& wContainerIdx, const wstring& wstrBufferTag, const _ulong& dwFlags /*= 0*/)
{
...
_int CTricksterDoor::Update_GameObject(const _float& fTimeDelta)
{
	switch (m_eCurState)
	{
	case CTricksterDoor::OPEN:
...
_int CTricksterDoor::LateUpdate_GameObject(const _float& fTimeDelta)
{
...
void CTricksterDoor::Render_GameObject(const _float& fTimeDelta)
{
	switch (m_eCurState)
	{
	case CTricksterDoor::OPEN:
...
void CTricksterDoor::Open()
{
	if (m_eCurState != OPENED)
		m_eCurState = OPEN;
	m_pMeshCom->Set_AnimationSet(0, 0.05);
	Engine::Play_Sound(L"prop_door_trickster_open.ogg", Engine::CSoundMgr::EFFECT);
	...
	dynamic_cast<CCutSceneCam*>(Engine::Get_MainCamera())->Set_Angle(&_vec3(0.f, D3DX_PI, 0.f));
}

CTricksterDoor * CTricksterDoor::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _ushort & wContainerIdx, const wstring & wstrBufferTag, const wstring & wstrNameTag, vector<_ulong>* pNaviIndices)
{
	CTricksterDoor* pInst = new CTricksterDoor(pGraphicDev);
