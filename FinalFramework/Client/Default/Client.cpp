                     ...
                     _In_ int       nCmdShow)
{
	...
	while (true)
	{
		...
		else
		{
			...
			if (!g_bFrameLimit || Engine::IsPermit_Call(L"Frame60", fTimer_Immediate))
			{
				Engine::Set_TimeDelta(L"Timer_FPS : 60");

				_float fTimer_FPS60 = Engine::Get_TimeDelta(L"Timer_FPS : 60");

				_int iRet = pMainApp->Update_MainApp(fTimer_FPS60 * g_fTime);
				if (1 == iRet)
					continue;
				pMainApp->LateUpdate_MainApp(fTimer_FPS60 * g_fTime);
				pMainApp->Render_MainApp(fTimer_FPS60 * g_fTime);
