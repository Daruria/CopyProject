...
public:
	static CAnimatedObject* Create(const wstring& wstrObjectKey, const wstring& wstrStateKey, const WORD iCnt, const D3DXVECTOR3& vPos,
		const D3DXVECTOR3& vScale = D3DXVECTOR3(1.f, 1.f, 0.f), bool bRenderListPush = false, float fDegree = 0.f, float fSort = 1.f, RENDER_TYPE eType = RENDER_OBJECT, CToolView* pView = nullptr);
