...
void CTerrain::Render()
{
	...
	int iLeftEdge	= static_cast<int>(m_pView->GetScrollPos(0));
	int iRightEdge	= static_cast<int>(m_pView->GetScrollPos(0)) + WINCX;
	int iTopEdge	= static_cast<int>(m_pView->GetScrollPos(1));
	int iBottomEdge = static_cast<int>(m_pView->GetScrollPos(1)) + WINCY;
...
HRESULT CTerrain::Initialize()
{
	...
	HRESULT hr = 0;
	CTexture* pTexture = m_pTextureMgr->GetTexture(L"Terrain");
	LPDIRECT3DDEVICE9 pGraphicDev = m_pDeviceMgr->GetDevice();
	if (!pTexture)
	{
		hr = m_pTextureMgr->LoadTexture(pGraphicDev, TEXTURE_MULTI, L"../Texture/Terrain/Option/Option (%d).png", L"Terrain", L"Option", 14, 1);
		FAILED_CHECK_MSG_RETURN(hr, L"TileOption LoadTexture Failed", E_FAIL);
	}
	else
	{
		vector<TEX_INFO*>* pvecOption = dynamic_cast<CMultiTexture*>(pTexture)->GetStateVector(L"Option");
		if (!pvecOption)
		{
			hr = m_pTextureMgr->LoadTexture(pGraphicDev, TEXTURE_MULTI, L"../Texture/Terrain/Option/Option (%d).png", L"Terrain", L"Option", 14, 1);
			FAILED_CHECK_MSG_RETURN(hr, L"TileOption LoadTexture Failed", E_FAIL);
...
void CTerrain::ImgListUpdate()
{
	for (auto iter = m_ImgList.begin(); iter != m_ImgList.end(); )
	{
		if ((*iter)->GetIsDead())
		{
			SafeDelete((*iter));
			iter = m_ImgList.erase(iter);
		}
		else
			++iter;
	}
...
{	
	...
	for (int i = 0; i < TILEX * TILEY; ++i)
	{
		...
		ReadFile(hFile, &byOptionID, sizeof(BYTE), &dwBytes, nullptr);
		ReadFile(hFile, &byBin, sizeof(BYTE), &dwBytes, nullptr);
		ReadFile(hFile, &byBin, sizeof(BYTE), &dwBytes, nullptr);

		TILE_INFO* pTileInfo = new TILE_INFO;
		pTileInfo->vPos = vPos;
		pTileInfo->vSize = vSize;
		pTileInfo->byDrawID = byDrawID;
		pTileInfo->byOption = byOptionID;

		//m_vecTile.push_back(new TILE_INFO(tTile));
		m_vecTile.push_back(pTileInfo);
	}
	...
	for (int i = 0; i < iSize; ++i)
	{
		...
		case CImageObject::ANIM_OBJ:
		{
			...
			CAnimatedObject* pAnimObj = CAnimatedObject::Create(wstring(ObjectKey.begin(), ObjectKey.end()), wstring(StateKey.begin(), StateKey.end()),
				static_cast<WORD>(tFrame.fMaxFrame), tInfo.vPos, tInfo.vScale, bRenderPush, fDegree, fSort, eRenderType, m_pView);
		...
		case CImageObject::NON_ANIM_OBJ:
		{
			...
			CNonAnimatedObject* pNonAnimObj = CNonAnimatedObject::Create(wstring(ObjectKey.begin(), ObjectKey.end()), wstring(StateKey.begin(), StateKey.end()),
				tInfo.vPos, tInfo.vScale, bRenderPush, fDegree, fSort, iDrawID, eRenderType, m_pView);
...
list<CImageObject*>* CTerrain::GetImgList()
{
	return &m_ImgList;
}
