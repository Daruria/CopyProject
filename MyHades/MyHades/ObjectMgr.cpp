...
void CObjectMgr::ObjectListClear(OBJECT_TYPE eType)
{
...
int CObjectMgr::Update()
{
	...
	//CCollisionMgr::CollisionObjectToTerrain(&m_ObjectList[OBJECT_PLAYER], &m_ObjectList[OBJECT_TERRAIN]);
	CCollisionMgr::CollisionObjectToTerrain(&m_ObjectList[OBJECT_PLAYER], dynamic_cast<CTerrain*>(m_ObjectList[OBJECT_TERRAIN].front())->GetVecTile());
	CCollisionMgr::CollisionObjectToTerrain(&m_ObjectList[OBJECT_MONSTER], dynamic_cast<CTerrain*>(m_ObjectList[OBJECT_TERRAIN].front())->GetVecTile());
	CCollisionMgr::CollisionHitBoxes(&m_ObjectList[OBJECT_PLAYER_HITBOX], &m_ObjectList[OBJECT_MONSTER]);
	CCollisionMgr::CollisionHitBoxes(&m_ObjectList[OBJECT_MONSTER_HITBOX], &m_ObjectList[OBJECT_PLAYER]);
...
void CObjectMgr::Render()
{
	for (int i = 0; i < RENDER_END; ++i)
	{
		m_RenderList[i].sort(CLayerSortStd());

		if (RENDER_SCREEN == i)
		{
			CMathMgr::DarkAmbienceRender();
		}
...
CTerrain * CObjectMgr::GetTerrain()
{
	if (m_ObjectList[OBJECT_TERRAIN].empty())
		return nullptr;

	return dynamic_cast<CTerrain*>(m_ObjectList[OBJECT_TERRAIN].front());
}

void CObjectMgr::SaveData(HANDLE hFile, const TCHAR * pFilePath)
{
...
void CObjectMgr::LoadData(HANDLE hFile, const TCHAR * pFilePath)
{
	if (m_ObjectList[OBJECT_TERRAIN].empty())
		return;

	dynamic_cast<CTerrain*>(m_ObjectList[OBJECT_TERRAIN].front())->LoadData(hFile, pFilePath);
